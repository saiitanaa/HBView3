    ...
    for child in node.named_children(&mut cursor) {
        match child.kind() {
            "expression_statement" => {
                ...
                let arguments = parse_call_arguments(
                    call,
                    source,
                )?;
            ...
            "declaration" => {
                if let Some(statement) = parse_declaration(child, source)? {
...
fn parse_call_arguments(
    ...
    for argument in arguments_node.named_children(&mut cursor) {
        arguments.push(parse_expression(argument, source)?);
    }
        ...
        "number_literal" => {
                ...
                .or_else(|| text.strip_prefix("0X"))
            {
                i64::from_str_radix(hex, 16)
                    .map_err(|error| {
                        format!(
                            "Failed to parse hexadecimal literal '{text}': {error}"
                        )
                    })?
            } else {
                text.parse::<i64>()
                    .map_err(|error| {
                        format!(
                            "Failed to parse integer literal '{text}': {error}"
                        )
                    })?
            };
